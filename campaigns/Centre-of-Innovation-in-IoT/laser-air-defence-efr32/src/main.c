...
#include "em_emu.h"
#include "em_gpio.h"
#include "sl_sleeptimer.h"   /* Gecko SDK sleep timer */
...
{
    g_tick_ms++;
    actuator_tick_ms();   /* laser burst / cooldown timers */
}
...
{
    /* EMU_GetTemperature() returns float Â°C â€“ requires EMU init   */
...
int main(void)
{
    ...
    /* â”€â”€ Kalman filters: one per axis â”€â”€ */
    ...
    bool     laser_on       = false;
    float    dt             = (float)CONTROL_LOOP_MS / 1000.0f;  /* seconds */

    /* â”€â”€ Safe startup: centre servos â”€â”€ */
    ...
    while (1) {
        ...
        /* â”€â”€ 1. Read & fuse sensors â”€â”€ */
        Target local = sensors_read_fused();

        /* â”€â”€ 2. Accept mesh target from peer nodes (if local not valid) */
        Target mesh_t = { 0 };
        bool   got_mesh = mesh_receive_target(&mesh_t);

        /* Choose best source: local confidence wins, else mesh */
        Target *best = &local;
        if (!local.valid && got_mesh && mesh_t.valid)
            best = &mesh_t;

        /* â”€â”€ 3. Kalman predict â”€â”€ */
        kalman_predict(&kf_az, dt);
        kalman_predict(&kf_el, dt);

        /* â”€â”€ 4. Kalman update if target valid â”€â”€ */
        float cmd_az = 0.0f;
        float cmd_el = 0.0f;
        ...
        if (best->valid) {
            ...
            /* Lead-target: predict position 50 ms ahead (servo latency) */
            cmd_az = kalman_lead_angle(&kf_az, 0.05f);
            cmd_el = kalman_lead_angle(&kf_el, 0.05f);

            (void)az_est; (void)el_est;  /* used internally by KF */

            /* â”€â”€ 5. Drive servos â”€â”€ */
            servo_set_pan_deg(cmd_az);
            servo_set_tilt_deg(cmd_el);

            /* â”€â”€ 6. Fire laser if confirmed + safe â”€â”€ */
            laser_on = laser_fire(true);

            /* â”€â”€ 7. Broadcast our target over mesh â”€â”€ */
            mesh_broadcast_target(best);

        } else {
            /* No target â€“ safe position, laser off */
            laser_fire(false);
            laser_on = false;

            /* Slowly return to centre (1Â°/tick) */
            ...
            /* Re-init KF to drift toward centre */
            kf_az.x = cur_az;
            kf_el.x = cur_el;
        }

        /* â”€â”€ 8. BLE telemetry every 500 ms â”€â”€ */
        if ((tick_now() - last_telem_ms) >= TELEMETRY_MS) {
            last_telem_ms = tick_now();
            telemetry_send(best, laser_on, mcu_temp());
        }

        /* â”€â”€ Sleep until next 20 ms deadline â”€â”€ */
